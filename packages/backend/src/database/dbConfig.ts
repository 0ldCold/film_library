...
const checkValidConfigWithError = (envConfig: DbConfigFormEnv): envConfig is Record<keyof ValidDbConfig, string> => {
  if (!isExist(envConfig)) {
    throw new DbConfigError('Конфиг не найден. Возможно не существует .env файл. '+
  ...
  if (!isExist(envConfig.host)) {
    throw new DbConfigError('В конфиге (.env) отсутствует поле VITE_DB_HOST. Попробуйте установить значение "localhost". ' +
      'Это значение должно совпадать с адресом, на котором хостуется PostgreSQL');
  }
  if (!isExist(envConfig.port)) {
    throw new DbConfigError('В конфиге (.env) отсутствует поле VITE_DN_PORT. Попробуйте установить значение вида "5432". ' +
    'Это значение должно совпадать с портом, на котором хостуется PostgreSQL');
  }
  if (Number.isNaN(+envConfig.port)) {
    throw new DbConfigError('В конфиге (.env) поле VITE_DN_PORT не является числом. Попробуйте установить значение вида "5432". ' +
      'Это значение должно совпадать с портом, на котором хостуется PostgreSQL');
  }
  if (!isExist(envConfig.username)) {
    throw new DbConfigError('В конфиге (.env) отсутствует поле VITE_DB_USERNAME. Тут должно быть имя пользователя в PostgreSQL');
  }
  if (!isExist(envConfig.password)) {
    throw new DbConfigError('В конфиге (.env) отсутствует поле VITE_DB_PASSWORD. Тут должен быть пароль пользователя в PostgreSQL');
  }
  if (!isExist(envConfig.database)) {
    throw new DbConfigError('В конфиге (.env) отсутствует поле VITE_DB_NAME. Тут должно быть название таблицы в PostgreSQL');
  }
...
if (!checkValidConfigWithError(dbConfigFormEnv)) {
  throw new DbConfigError("Проверьте содержимое .env файла. " +
